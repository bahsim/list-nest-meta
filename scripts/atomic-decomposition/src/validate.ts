...
export function validateAtomicIntents(dir: string): void {
  ...
  for (const file of files) {
    ...
    doc.atomic_intents.forEach((intent: any, idx: number) => {
      ...
      // Validate parent_intent_id and parent_file
      if (intent.parent_intent_id !== getExpectedParentId(file)) {
        console.error(`${prefix} parent_intent_id does not match expected parent intent id (${getExpectedParentId(file)})`);
        hasErrors = true;
      }
      if (intent.parent_file && path.basename(intent.parent_file) !== getExpectedParentFile(file)) {
        console.error(`${prefix} parent_file does not match expected parent file (${getExpectedParentFile(file)})`);
        hasErrors = true;
      }
    });
  }
...
// Helper functions for parent validation
function getExpectedParentId(file: string): string {
  // Assumes atomic file is named <parentIntentId>-atomic.yaml
  return file.replace(/-atomic\.ya?ml$/, "");
}
function getExpectedParentFile(file: string): string {
  // Assumes parent YAML is in ../07-unified-intent or similar
  // This is a best-effort guess; adjust as needed for your structure
  return file.replace(/-atomic\.ya?ml$/, ".yaml");
}
